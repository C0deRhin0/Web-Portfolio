interface ClearingLoaderOptions {
...
interface ClearingLoaderReturn {
  start: () => void;
  stop: () => void;
}
...
const getColoredClearingText = (spinningChar: string): string => {
  // Color codes: #cfcfcf for "clearing", #dda520 for spinning character
  return `\x1b[38;2;207;207;207mclearing... \x1b[38;2;221;165;32m${spinningChar}\x1b[0m`;
};

export const createClearingLoader = (
  writeFunction: (text: string) => void,
  options: ClearingLoaderOptions = {}
): ClearingLoaderReturn => {
  const {
  ...
  const updateSpinningChar = () => {
    ...
    const coloredText = getColoredClearingText(spinningChar);
